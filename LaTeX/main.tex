...
\end{proposition}
\begin{proof}
    Soit $\varepsilon<1/M$, montrons que $f_\varepsilon$ est bien une densité.
    Sur $[0,1]$,
...
%\end{proof}
    ...
    Pour la construction nous utilisons les polynômes de Jacobi.
    Toutefois au vu de la preuve, nous pouvons utiliser 
    n'importe quelle familles de polynômes dits orthogonaux.
    \textcolor{red}{C'est vrai ça pourquoi Jacobi et pas tchbeychev ou legendre ??
    Pour avoir choisi ceux-là ? toruve une raison pratique,
    j'ai pas internet pour chercher.}

    \textcolor{red}{en admettant qu'on s'interesse à beta(alpha, 2-alpha),
    ca sera motivé dans la derniere section, pourquoi ici on ne peut pas considerer
    alpha entre 0 et 1 ?? Faut expliquer, donner une justification.
    Sinon c'ewst pas très gave, on le mentionne pas e ton fait ocmme si rien n'était 
    mais bizarre de pas prendre des plus petit alpha.}
    ...
    \textcolor{red}{J'ai pas eu le temps (ni internet) pour reformuler cette partie comme
    je souhaterai le faire (remarque WhatsApp) }
    \textcolor{red}{Dire que beta a été simulé par trapeze ? montecarlo ? faire quelques
    phrase dessus}
    La figure met clairement en évidence le rôle crucial de la borne $\varepsilon_n$ , qui garantit que la densité perturbée $f_\varepsilon$ reste positive sur $[1,0]$
    et définit bien une mesure de probabilité. Cette borne dépend directement du degré $n$ du polynôme de jacobi \cite{Szego1939}, 
    $$\varepsilon_n\sim\;\frac{C(\alpha)}{n^{|\alpha-1|}}$$
    où $C(\alpha)$ provient du comportement asymptotique des polynômes de Jacobi.
    Dans le cas, $\alpha=1$, la mesure $\Lambda_0^1$ est uniforme est $\varepsilon_n$ ne depend plus de n, d'où sur la figure les histogrammes empiriques collent partout.
    En revanche, pour $\alpha=1.4$,  $\varepsilon_n=0.392062$, et sur la figure on voit que si on prend $\varepsilon$ plus grand que $\varepsilon_n$ les histogrammes empiriques 
    ne se superposent plus parfaitement. 
...
%\end{figure}
    ...
    L'étude de Niwa et al. (2016) sur la sardine japonaise (\textit{Sardinops 
    melanostictus}) constitue une démonstration empirique de la nécessité du 
    $\Lambda$-coalescent. Elle met en lumière l'échec du modèle de Kingman pour
     les espèces à stratégie de "Sweepstakes" ou "loterie reproductive" (haute 
     fécondité, variance reproductive infinie), où quelques rares individus 
     peuvent engendrer une fraction macroscopique de la génération suivante 
     (fusions multiples). 

    L'idée de cette sous-partie est d'analyser une possible application du 
    modèle de $\Lambda$-coalescence sur la mesure Beta$(2-\alpha, \alpha)$ au 
    monde réel.

    \subsection{Le Conflit}

    Chez les animaux à haute fécondité, l'analyse génétique révèle une contradiction 
    flagrante si l'on reste dans le paradigme binaire classique (Wright-Fisher ou Kingman) :

    On observe un excès massif de mutations uniques. Sous Kingman, cette 
    observation est interprétée à tort comme une expansion démographique 
    explosive et récente.

    Si cette expansion était réelle, la distribution des distances par paires 
    (mismatch distribution) devrait former une "vague" unimodale (loi de Poisson). 
    Or, les données montrent une distribution en forme de "L" : un pic en zéro et 
    une queue lourde.

    \subsection{Résolution par le Beta-Coalescent et Simulation}

    Le modèle Beta-coalescent (avec $\alpha \approx 1.3$) réconcilie ces deux signaux. 
    Les fusions multiples fréquentes créent des groupes d'individus identiques (le pic à 
    0), tout en laissant survivre des lignées très anciennes (queue lourde), sans recourir 
    à l'hypothèse d'expansion.

    Nous avons simulé cette distinction (Fig. \ref{fig:sardine_simu}) en comparant un 
    modèle de Kingman avec expansion (l'artefact) et un modèle Beta stationnaire (la réalité).

    \begin{figure}[H]
        \centering
        \includesvg[width=\textwidth]{photosweep}
        \caption{Distribution des discordances ($n=150$). \textbf{En bleu (Kingman + Expansion)} 
        : Une "vague" créée par le phénomène de croissance démographique. \textbf{En rouge 
        (Sardine $\alpha=1.3$)} : Un pic de clones (sweepstakes récents) et une queue de lignées 
        persistantes (résultat attendu).}
        \label{fig:sardine_simu}
    \end{figure}

    \subsection{Conséquence : Millions de sardines, même génome}

    Ce résultat invalide la relation linéaire entre taille de recensement $N$ et 
    taille efficace $N_e$. Sous le régime Beta-coalescent, la dérive génétique est régie par 
    une loi de puissance :
    $$ N_e \propto N^{\alpha - 1} $$
    Pour la sardine ($\alpha \approx 1.3$), cela implique $N_e \propto N^{0.3}$. Cette 
    relation explique pourquoi des populations marines gigantesques conservent une diversité 
    génétique très faible et instable. Ce modèle permet non seulement d'expliquer un ancien 
    paradoxe chez les animaux à haute fécondité mais sert en plus d'outil pour estimer la taille 
    effective d'une population.
