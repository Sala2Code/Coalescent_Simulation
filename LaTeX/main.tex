...
%\end{figure}
    ...
    \textcolor{blue}{Je propose une trame car celle actuelle je 
    la trouve pas très intuitive par rapport au rapport. Les textes 
    que j'écris en rouge c'est plus une indication qu'une vraie formulation (ou des remarques à completer)}

    \textcolor{red}{[Précédemment nous avons introduit la loi
    $Beta(2-\alpha, \alpha)$, celle-ci est largement utilisée en
    biologique car TODO + citation]}
    \textcolor{red}{En effet le modèle simple de Kingman n'est pas suffisant.
    Les sardines japonaises [NIWA] [mettre la reference 
    dans le .bib et faire un \\cite\{\} ] l'illustre bien.}
    
    L'étude de Niwa et al.\ (2016) sur la sardine japonaise constitue une démonstration empirique de la
nécessité du $\Lambda$-coalescent. Elle illustre l'échec du modèle de Kingman pour les espèces à stratégie
dite de \textit{sweepstakes}, dans lesquelles seuls quelques individus parmi des millions réussissent à
...
\[
\mathbb{P}(X \ge x) \sim C\, x^{-\alpha}, \qquad 1 < \alpha < 2,
\]
...
rigoureusement le phénomène de \textit{sweepstakes}.
    ...
    \begin{figure}[H]
        ...
        clones et une queue de lignées 
