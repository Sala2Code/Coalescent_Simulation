...
\maketitle
    ...
    La grande partie des coalescences se font dans les premiers instants.
    En référence au lemme des réveils, le nombre de paires possibles, et donc 
    de réveils prêts à sonner et paramétrés de la même manière, sont plus nombreux 
...
\label{fig:tmrca_foret}
    ...
    et $\varepsilon_n$
    ne depend plus de $n$. Toutefois, le choix de $\varepsilon$ est tout de même borné
...
Afin de justifer le modèle du $\Lambda$-coalescent, comparé à 
...
$i\neq j$ leur discordance, noté $K_{ij}\in \mathbb N$, c'est le nombre
...
Sous des hypothèses biologiques
naturelles, nous avons que les mutations ne peuvent pas
se produire sur le même site et que pour un paramètre 
de mutation constant $\mu>0$, pour tout $t>0$
\textcolor{red}{une citation pour cette définition de Poisson et hypothese}
\[
...
Comme choix de mesure couramment utilisés en biologie est le 
modèle de Kingman avec expansion. % source ?
C'est-à-dire que l'intensité des coalescences varie 
au cours du temps.
Ce modèle permet d'expliquer des populations où le nombre
de descendants sont modérés par rapport à la taille de la population totale.
...
et supposons que notre population % introduire le lien avec queue lourde
a une reproduction à queue lourde, c'est-à-dire qu'un individu 
peut être à l'origine d'une partie non négligeable de la population. % citation nécessaire ! car pourquoi une queue lourde ferait ça et pas autre chose?
% Cette dernière 
...
\begin{figure}[H]
        \centering
        \includesvg[width=\textwidth]{photo/plot_Kij_overlap_retry}
        \caption{TODO: en gros on regarde et bah c'est pareil donc on peut rien déduire}
...
Pour l'expérience numérique nous prenons $\alpha=1.33$ comme dans
\cite{Niwa2016} qui a été choisi comme le paramètre 
maximisant la vraisemblance. 
Puis pour le modèle de Kingman avec expansion, nous 
...
% à quel point c'est utile etc...
  ...
  \begin{figure}[H]
    ...
    \textcolor{red}{Parler un peu plus du plot,
    de la construction de l'intervalle de confiance, de la simualtion,
    de la déterminaition des paramètres}
