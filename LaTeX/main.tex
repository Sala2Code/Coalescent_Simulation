...
\begin{proposition}\label{prop:beta_indiscernable}
    ...
    de densité $w_\alpha(x)$ sur $[0,1]$ \footnote{\[
    w_\alpha(x)
    = \frac{1}{B(2-\alpha,\alpha)}\, x^{1-\alpha}(1-x)^{\alpha-1},
    \qquad x\in[0,1]
\]
}.
    Soit $Q_{k}$ un polynôme de Jacobi de degré $k$,
    orthogonal à tous les polynômes de degré $\le n-2$ pour
    \[
...
\end{proposition}
\begin{proof}
    Montrons que pour $\varepsilon$ assez petit, $f_\varepsilon$ est bien une densité
    de probabilité.En prenant $\varepsilon<1/M$, on a
    \[
