...
%\end{figure}
    ...
    L'étude de Niwa et al. (2016) sur la sardine japonaise 
    (\textit{Sardinops melanostictus}) constitue une démonstration 
    empirique de la nécessité du $\Lambda$-coalescent. 
    
    Elle illustre l'échec du modèle de Kingman pour les espèces 
    à stratégie de ``Sweepstakes'' (en français ``loterie reproductive''), 
    caractérisée par une haute fécondité et une haute mortalité 
    (donc une grande variance), où quelques rares individus peuvent 
    engendrer une fraction macroscopique de la génération suivante.

    L'objectif de cette sous-partie est d'analyser une possible 
    application du modèle de $\Lambda$-coalescence sur la mesure 
    Beta$(2-\alpha, \alpha)$ au monde réel.
    ...
    classique (Wright-Fisher ou Kingman) :

    On observe un excès massif de mutations uniques. Sous Kingman, 
    cette observation ne peut être interprétée que comme une 
