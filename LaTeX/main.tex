...
\maketitle
    ...
    En référence au lemme des réveils, le nombre de paire possible, et donc 
...
\subsection{Conséquence : Millions de sardines, même génome}
    ...
    Ce résultat ilustre le probleme de la coalescence de kingman et montre qu'une population 
    immense ($N$) ne possède pas nécessairement une grande diversité génétique. 
    Dans le Beta-coalescent, la 
    population est régie par une loi de puissance:
    $$ \text{Échelle} \propto N^{\alpha - 1} $$
    
    Avec $\alpha \approx 1.3$, on obtient une dépendance en $N^{0.3}$. 
    Cette relation non-linéaire pourquoi 
    des millions de sardines semblent partager le même génome, 
    se comportant comme une population minuscule.
    
    Ce modèle permet d'expliquer un ancien paradoxe 
    chez les animaux à haute fécondité et offre  
    un cadre théorique pour comprendre les dynamiques 
    à variance infinie.
